...
parser.add_argument('--resume', '-r', default='10sysu_deen_p4_n6_lr_0.1_seed_0_pconv_0626*_0.2_best.t', type=str, help='resume from checkpoint')
...
def extract_gall_feat(gall_loader):
    ...
    all_fc   = []
    with torch.no_grad():
        for batch_idx, (imgs, labels) in enumerate(gall_loader):
            imgs = imgs.cuda(non_blocking=True)
            feat_pool, feat_fc = net(imgs, imgs, test_mode[0])
            all_pool.append(feat_pool.detach().cpu().numpy())
            all_fc.append(  feat_fc.detach().cpu().numpy())
    # 拼接所有 batch
    gall_feat_pool = np.vstack(all_pool)
    gall_feat_fc   = np.vstack(all_fc)
...
def extract_query_feat(query_loader):
    ...
    all_fc   = []
    with torch.no_grad():
        for batch_idx, (imgs, labels) in enumerate(query_loader):
            imgs = imgs.cuda(non_blocking=True)
            feat_pool, feat_fc = net(imgs, imgs, test_mode[1])
            all_pool.append(feat_pool.detach().cpu().numpy())
            all_fc.append(  feat_fc.detach().cpu().numpy())
    query_feat_pool = np.vstack(all_pool)
    query_feat_fc   = np.vstack(all_fc)
    print('Extracting Time:\t {:.3f}'.format(time.time()-start))
