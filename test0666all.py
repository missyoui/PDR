...
parser.add_argument('--tvsearch', action='store_true', help='whether thermal to visible search on RegDB') # RegDB
#parser.add_argument('--test_mode', default=[1, 2], type=int, nargs=2, help='test mode: [1, 2] for IR to VIS, [2, 1] for VIS to IR')
parser.add_argument('--use-pconv', action='store_true', help='Use PConv module in ResNet backbone')
parser.add_argument('--llcmmode', default=1, type=int, help='1 or 2 for whether visible to thermal  search on llcm')
...
if dataset == 'llcm':
    ...
    # testing set
    query_img, query_label, query_cam = process_query_llcm(data_path, mode=args.llcmmode)
    gall_img, gall_label, gall_cam = process_gallery_llcm(data_path, mode=args.llcmmode, trial=0)
    ...
    for trial in range(10):
        gall_img, gall_label, gall_cam = process_gallery_llcm(data_path, mode=args.llcmmode, trial=trial)
