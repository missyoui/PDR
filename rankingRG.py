...
parser.add_argument('--data_path',default= '/home/bder73002/FSLai/AGW/LLCM/',type=str, help='./test_data')
...
for j in range(len(query_img)):
    ...
    if j % 10 == 0:
        ...
        print(query_path)
        print('Top 8 images are as follow:')
        # Visualize Ranking Result 
        # Graphical User Interface is needed
        import matplotlib.patches as patches

        topk = 8  # 展示Top-8
        real_topk = min(topk, len(index))  # 防止越界
        ...
        print("len(gall_img):", len(gall_img))
        print("len(index):", len(index))
        print("index[:10]:", index[:10])
        print("max(index):", max(index))
        print("min(index):", min(index))


        for i in range(real_topk):  # 用 real_topk 防止越界
            ...
            im = cv2.resize(im, (args.img_w, args.img_h))
            ax.imshow(im)
            edge_color = 'green' if int(label) == int(query_label) else 'red'
            rect = patches.Rectangle(
                (0, 0), args.img_w, args.img_h, linewidth=10,
                edgecolor=edge_color, facecolor='none'
            )
            ax.add_patch(rect)
            ax.set_title(f'{i+1}', fontsize=12)

        # 补齐空白（防止后面的subplot也越界）
        for i in range(real_topk, topk):
            ax = plt.subplot(1, 9, i + 2)
            ax.axis('off')


        fig.savefig(f"./save_ranking/show{j}.jpg", bbox_inches='tight')
