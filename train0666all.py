...
from model0611 import embed_net
from utils import *
from loss import OriTripletLoss, CPMLoss
from loss_sd import SDLoss
...
parser.add_argument('--lambda_3', default=0.2, type=float, help='weight for SD Loss')  #  Added SD Loss weight
parser.add_argument('--use-pconv', action='store_true', help='Use PConv module in ResNet backbone')
...
if dataset == 'sysu':
    data_path =  '/home/bder73002/FSLai/AGW/SYSU-MM01/'
    log_path = args.log_path + 'sysu_log/'
    test_mode = [1, 2]  # thermal to visible
    pool_dim = 2048
elif dataset == 'regdb':
    data_path = '/home/bder73002/FSLai/AGW/RegDB/'
    log_path = args.log_path + 'regdb_log/'
    test_mode = [2, 1]  # visible to thermal
    pool_dim = 1024
elif dataset == 'llcm':
    data_path = '/home/bder73002/FSLai/AGW/LLCM/'
...
suffix = suffix + '_pconv_0626*_0.1all' if args.use_pconv else suffix
...
net = embed_net(n_class, dataset, arch=args.arch, use_pconv=args.use_pconv)
net.to(device)
...
criterion_sd = SDLoss(delta=0.5, max_area=13000, use_mask=False).to(device)  #  Initialize SD Loss
...
def train(epoch):
    ...
    for batch_idx, (input1, input2, label1, label2) in enumerate(trainloader):
        ...
        loss_ort = loss_ort *  args.lambda_2
                ...
                # ==== Compute SD Loss based on feature norm (Scale regularization) ====
        feat_norm = torch.log1p(torch.norm(feat1, dim=1, keepdim=True))  # 
        pred_boxes = torch.cat([
            torch.zeros_like(feat_norm),
            torch.zeros_like(feat_norm),
            feat_norm,
            feat_norm
        ], dim=1)
        gt_boxes = torch.ones_like(pred_boxes)

        loss_sd = criterion_sd(pred_boxes, gt_boxes) * args.lambda_3

        loss = loss_id + loss_tri + loss_cpm + loss_ort+ loss_sd  #  include SD Loss
        optimizer.zero_grad()
        loss.backward()
        optimizer.step()
        ...
        sd_loss.update(loss_sd.item(), 2 * input1.size(0))
        total += labels.size(0)
        ...
        if batch_idx % 50 == 0:
            print('Epoch: [{}][{}/{}] '
                  ...
                  'SLoss:{sd_loss.val:.3f} '.format(
                epoch, batch_idx, len(trainloader),
                train_loss=train_loss, id_loss=id_loss, tri_loss=tri_loss, cpm_loss=cpm_loss, ort_loss=ort_loss, sd_loss=sd_loss))
    ...
    writer.add_scalar('sd_loss', sd_loss.avg, epoch)
