...
from resnet_pconvg import resnet50, resnet18  # 修改此处
...
class visible_module(nn.Module):
    def __init__(self, arch='resnet50', use_pconv=False):
        super(visible_module, self).__init__()
        model_v = resnet50(pretrained=True, last_conv_stride=1, last_conv_dilation=1, use_pconv=use_pconv)  # 添加 use_pconv
...
class thermal_module(nn.Module):
    def __init__(self, arch='resnet50', use_pconv=False):
        ...
        model_t = resnet50(pretrained=True, last_conv_stride=1, last_conv_dilation=1, use_pconv=use_pconv)  # 添加 use_pconv
...
class base_resnet(nn.Module):
    def __init__(self, arch='resnet50', use_pconv=False):
        ...
        model_base = resnet50(pretrained=True, last_conv_stride=1, last_conv_dilation=1, use_pconv=use_pconv)  # 添加 use_pconv
...
class CNL(nn.Module):
    def __init__(self, high_dim, low_dim, flag=0):
        ...
        self.g = nn.Conv2d(self.low_dim, self.low_dim, kernel_size=1)
        self.theta = nn.Conv2d(self.high_dim, self.low_dim, kernel_size=1)
        if flag == 0:
            self.phi = nn.Conv2d(self.low_dim, self.low_dim, kernel_size=1)
            self.W = nn.Sequential(nn.Conv2d(self.low_dim, self.high_dim, kernel_size=1),
                                   nn.BatchNorm2d(high_dim))
        else:
            self.phi = nn.Conv2d(self.low_dim, self.low_dim, kernel_size=1, stride=2)
            self.W = nn.Sequential(nn.Conv2d(self.low_dim, self.high_dim, kernel_size=1, stride=2),
...
class PNL(nn.Module):
    def __init__(self, high_dim, low_dim, reduc_ratio=2):
        ...
        self.g = nn.Conv2d(self.low_dim, self.low_dim//reduc_ratio, kernel_size=1)
        self.theta = nn.Conv2d(self.high_dim, self.low_dim//reduc_ratio, kernel_size=1)
        self.phi = nn.Conv2d(self.low_dim, self.low_dim//reduc_ratio, kernel_size=1)

        self.W = nn.Sequential(nn.Conv2d(self.low_dim//reduc_ratio, self.high_dim, kernel_size=1),
                               nn.BatchNorm2d(high_dim))
        nn.init.constant_(self.W[1].weight, 0.0)
        nn.init.constant_(self.W[1].bias, 0.0)
...
class embed_net(nn.Module):
    def __init__(self, class_num, dataset, arch='resnet50', use_pconv=False):
        super(embed_net, self).__init__()

        self.thermal_module = thermal_module(arch=arch, use_pconv=use_pconv)
        self.visible_module = visible_module(arch=arch, use_pconv=use_pconv)
        self.base_resnet = base_resnet(arch=arch, use_pconv=use_pconv)
